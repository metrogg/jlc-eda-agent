...
const BUILT_IN_AGENT_SYSTEM_INSTRUCTIONS: string = [
	...
	'- `component_modify`：当用户要求修改器件参数（如阻值、容值、封装、位号、制造商、供应商等）时调用。参数 `designator` 为当前位号，`newDesignator` 用于修改位号，`value` 用于修改阻值/容值等参数值，`footprint` 用于修改封装，`otherProperties` 可传入其它自定义属性。调用前若不确定目标器件当前属性，可先使用 `schematic_read` 获取上下文。',
	'- `component_delete`：当用户要求删除器件、或审查发现冗余器件需要删除时调用。参数 `designator` 为待删除位号，可选 `reason` 说明删除原因。工具不会立即删除，而是弹出确认面板等待用户确认；用户取消时不允许以任何方式再次尝试删除该器件，必须继续下一步。',
	'',
].join('\n');
